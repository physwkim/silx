...
class BenchmarkWidget(qt.QWidget):
    ...
    def _startNextBackend(self):
        ...
        self._label.setText(
            f"Benchmarking: {backend} | {self._n_points} points | "
            f"{self._duration}s"
        )
    ...
    def _update(self):
        ...
        plot.addCurve(self._x, y, legend="bench", color="blue",
                      linewidth=2, resetzoom=False)
        self._counter.tick()
    ...
    def _showResults(self):
        ...
        self._label.setText(" | ".join(
            f"{b}: {f:.1f} FPS" for b, f in self._results.items()
        ))
